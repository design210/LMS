				...
				<ul class="list">
					<li class="d-flex" v-for="(item, index) in setLectureList" :key="index">
						...
						<div class="info-group">
							...
							<div class="date"><v-icon>mdi-calendar-month</v-icon>{{ item.open_lecture_period }}</div>
							<div class="d-flex data-wrap">
								<div class="status">
									<div class="percent">{{ item.progress_rate_avg }}%</div>
									<div class="txt">평균진도율</div>
								</div>
						...
						<div class="person-wrap d-flex">
							<div class="person">{{ item.my_lecture_count }}명</div>
							<router-link :to="{ path: `/lecture/signuplecture/status/${item.idx}` }"
								><v-btn depressed color="primary"> 수강현황 </v-btn></router-link
							>
						</div>
					</li>
